...
class Camera:
    ...
    def take_picture(self, file_name):
        ...
        if not os.path.exists("images_6/"+file_name.split(".")[0]):
            os.makedirs("images_6/"+file_name.split(".")[0])
        time.sleep(2)
        cv2.imwrite("images_6/" + file_name.split(".")[0] + "/"+ file_name + "_6.jpg", frame)
        
        
camera = Camera()
camera.take_picture(sys.argv[1])
